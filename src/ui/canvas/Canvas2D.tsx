      ...
      ? (() => {
          ...
          return ([
            ...
            const dragBound = (pos: { x: number; y: number }) => {
              ...
              const stepMm = snapToGrid ? snapIncrementMm : MIN_INCREMENT_MM;
              const snapped = { xMm: snapMm(world.xMm, stepMm), yMm: snapMm(world.yMm, stepMm) };
              return worldToScreen(snapped, camera);
            };
            return (
              <Rect
                ...
                onDragEnd={(evt) => {
                  ...
                  const stepMm = snapToGrid ? snapIncrementMm : MIN_INCREMENT_MM;
                  const snappedWorld = { xMm: snapMm(worldPoint.xMm, stepMm), yMm: snapMm(worldPoint.yMm, stepMm) };
                  const localPoint = toLocalPoint(selectedDimension, snappedWorld);
                  const baseLocal = {
                    xMm: localPoint.xMm - normal.xMm * selectedDimension.offsetMm,
                    yMm: localPoint.yMm - normal.yMm * selectedDimension.offsetMm,
                  };
                  const patch = key === "start" ? { startMm: baseLocal } : { endMm: baseLocal };
