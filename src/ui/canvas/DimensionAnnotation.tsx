...
export default function DimensionAnnotation({
  ...
  const direction = useMemo(
    () => normalizeVector({ xMm: endMm.xMm - startMm.xMm, yMm: endMm.yMm - startMm.yMm }),
    [endMm, startMm],
  );
  ...
  const isVertical = Math.abs(direction.yMm) >= Math.abs(direction.xMm);
  const textRotation = -rotationDeg + (isVertical ? -90 : 0);
  ...
  const getLocalPoint = (absolutePos: { x: number; y: number }) => {
    if (!groupRef.current) return absolutePos;
    ...
    return inverted.point(absolutePos);
  };

  const toAbsolutePoint = (localPos: { x: number; y: number }) => {
    if (!groupRef.current) return localPos;
    const parent = groupRef.current.getParent?.();
  ...
  const getSnappedOffsetMm = (absolutePos: { x: number; y: number }) => {
    const localPos = getLocalPoint(absolutePos);
